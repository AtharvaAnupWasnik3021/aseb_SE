...
class BiologyAgent:
    ...
    def _no_evidence_result(self, reason: str, ensembl_id=None) -> dict:
        """Return a properly-shaped AVOID result when biological evidence is absent.
        The pipeline continues; the low score and AVOID verdict trigger a REJECT."""
        return {
    ...
    def run(self):
        ...
        if not ensembl_id:
            return self._no_evidence_result(
                f"Gene '{self.gene_symbol}' not found in Open Targets"
            )

        associations = get_disease_associations(ensembl_id)
        if not associations:
            return self._no_evidence_result(
                f"No disease associations found for '{self.gene_symbol}' in Open Targets",
                ensembl_id=ensembl_id,
            )
        ...
        if not match:
            return self._no_evidence_result(
                f"No association between '{self.gene_symbol}' and '{self.disease_name}' in Open Targets",
                ensembl_id=ensembl_id,
            )
